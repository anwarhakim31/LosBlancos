...
const ModalEditDiskon = ({ onClose, callback, isEditData }: PropsType) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      percent: isEditData?.percent || null,
      info: isEditData?.info || "",
    },
  });
  ...
  const onSubmit = async (data: {
    ...
    setLoading(true);
    try {
      const res = await diskonService.edit({
        code: data.code,
        percent: data.percent as number,
        info: data.info,
        id: isEditData?._id as string,
      });
...
export default ModalEditDiskon;
